...
- **`http_interval_ms` INI key** — configurable dashboard refresh interval (500–30000 ms, default 2000). Increase for overnight monitoring to reduce phone battery drain.
...
- **HDR mode indicator on dashboard** — the AGC card changes label to HDR and shows "HDR enabled" when `hdr_enable = 1` (RSPdx only).
...
- Multi-recording schedule: `sdrplay_api_Update` was being called before stream re-initialisation between recordings. Frequency update now only calls the live API when the stream is running; otherwise updates config for pickup on next `sdrplay_api_Init`.
- Multi-recording schedule: elapsed time and file size now reset correctly between entries on the remote dashboard.
- Multi-recording schedule: dangling `ch_a_params` pointer after `sdrplay_api_ReleaseDevice` could cause access violation when HTTP worker thread polled after recording ended. Both pointers are now nulled on device release.
...
- Drive spin-up pre-write (`spinup_enable`, `spinup_bytes`) to wake idle spinning hard disks before recording begins, preventing ring buffer overflows at the start of long sessions.
